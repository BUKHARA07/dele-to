  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const recipient of recipientsToProcess) {
      if (recipient.requirePassword && !recipient.password.trim()) {
        setError(
          `Please enter a password for ${recipient.name} or disable password protection`
        );
        return;
      }
    }
    ...
    try {
      ...
      for (const recipient of recipientsToProcess) {
        ...
        if (result.success && result.id) {
          ...
          setError(
            result.error ||
              `Failed to create secure share for ${recipient.name}`
          );
          return;
        }
      }
          ...
          <Card>
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-2xl">
                {formData.multiRecipient
                  ? `${generatedLinks.length} Secure Links Created!`
                  : "Secure Link Created!"}
              </CardTitle>
              <CardDescription>
                Your secret has been encrypted client-side and is ready to share
                with{" "}
                {formData.multiRecipient
                  ? "multiple recipients"
                  : "your recipient"}
            ...
            <CardContent className="space-y-6">
                    ...
                    >
                      <CardHeader className="pb-3">
                        ...
                        <CardDescription className="text-sm">
                          Expires: {link.expirationTime} • Max views:{" "}
                          {link.maxViews}
              ...
              <Alert>
                ...
                <AlertDescription>
                  <strong>Security Notice:</strong> The decryption key is
                  included in the URL fragment (#) and never sent to our
                  servers. Only share each complete link with its intended
                  recipient.
                </AlertDescription>
              </Alert>
              ...
              <Alert>
                ...
                <AlertDescription>
                  <strong>Important:</strong> Each link will expire based on its
                  individual settings. Your data is encrypted with AES-256 and
                  can only be decrypted by someone with the complete link.
                </AlertDescription>
              </Alert>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="100">Unlimited views</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-gray-500 mt-1 hidden md:block">
                          Multi-recipient, access controls & more
                        </p>
                        <p className="text-xs text-gray-500 mt-1 md:hidden">
                          Multi-recipient and access controls
                        </p>
                      </div>
                    </div>
                    <ChevronDown
                        ...
                        {singleRecipientSettings.requirePassword && (
                          <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                              Click the refresh button to generate a secure
                              random password. Use the eye icon to view the
                              password.
                            </p>
                          </div>
                        )}
